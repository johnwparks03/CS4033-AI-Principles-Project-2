...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for boardPosition in boardPositions:
        manhattanDistance = boardPosition[0] + boardPosition[1]
        manDistMinusNoise = manhattanDistance - MAX_NOISE
        manDistPlusNoise = manhattanDistance + MAX_NOISE
        if manDistMinusNoise >= 0 and manDistMinusNoise not in observations:
            observations.append(manDistMinusNoise)
        if manDistPlusNoise not in observations:
            observations.append(manDistPlusNoise)


    variableDomainsDict[PAC] = boardPositions
    variableDomainsDict[GHOST0] = boardPositions
    variableDomainsDict[GHOST1] = boardPositions
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            factors, newFactor = joinFactorsByVariable(factors, variable)

            if len(newFactor.unconditionedVariables()) > 1:
                factors.append(eliminate(newFactor, variable))
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            posProb = self.beliefs[pos]
            for newPos in newPosDist.keys():
                newDist[newPos] += posProb * newPosDist[newPos]
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        while numParticles > 0:
            if numParticles > len(legalPositions):
                ...
                numParticles -= len(legalPositions)
            else:
                self.particles += legalPositions[0 : numParticles]
                numParticles = 0
